...
config = load_config("ocr_config_multirow.yaml")
# config = load_config()


class LPReaderModel(nn.Module):
    def __init__(self, in_channels=3, out_channels=32, num_classes=len(config["data"]["classes"]), loc_conv1_out_channels = 32,
        ...
        self.classifier = nn.Conv2d(out_channels * 16, num_classes, kernel_size=1)
    ...
    def stn(self, x):
        ...
        grid = F.affine_grid(theta, ([x.size()[0], 3, config["data"]["img_target_height"], config["data"]["img_target_width"]])) # N x C x H x W
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        x = x.view(x.size(0), config["data"]["batch_size"]*8, 1, config["data"]["max_lp_length"])
